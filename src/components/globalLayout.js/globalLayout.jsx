        ...
        <Menu
          ...
          items={[
            ...
            },
            {
              ...
              label: <p className={`text-lg ${!collapsed ? "text-black": "text-white"}`}> publico </p>,
        ...
        <div className='flex  justify-center items-center bg-gray-50  '>
          <form onSubmit={sendMensage} className='w-11/12 flex  gap-4 flex-nowrap justify-center bg-gray-50  itens-center'>
            <TextArea onClick={() => setShowScrow(true)} onChange={(e) => setMensage(e.target.value)}
              value={mensage}
              className="mb-5 text-gray-700 font-semibold w-full pt-3 p-3 flex flex-col  rounded-3xl shadow-2xl border-y-2 border-gray-300 bg-gray-50  max-h-s sm:w-[60%] 2xl:w-[40%] lg:w-[50%]"
              autoSize={{ minRows: 1, maxRows: 50 }}
              onKeyUp={handleKeyPress}
              rows={1} placeholder="Mensagem datachat" maxLength={5000}
              
            />
            {false &&
              <button className=' flex  justify-center mt-1 h-12 w-8'>

                <SendOutlined className='hover:text-green-400' style={{ color: "#1677ff", fontSize: "38px" }} />
              </button>
            ...
            { false &&  <Tooltip color={`${!showScrow ? "blue" : "red"}`} title={`${!showScrow ? "Scroll ativado" : "Scroll desativado"}`}>
